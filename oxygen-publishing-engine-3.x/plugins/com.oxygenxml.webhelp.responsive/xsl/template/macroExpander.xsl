...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="wh-macro">
        ...
        <xsl:choose>
            ...
            <xsl:when test="'oxygen-webhelp-transformation-timestamp' eq $name">
                <xsl:call-template name="wh-macro-oxygen-transformation-timestamp"/>
            </xsl:when>
    ...
    <!-- Expands the 'oxygen-webhelp-transformation-timestamp' macro. -->
    <xsl:template name="wh-macro-oxygen-transformation-timestamp">
        <xsl:value-of select="$WEBHELP_TRANSFORMATION_TIMESTAMP"/>
    </xsl:template>
