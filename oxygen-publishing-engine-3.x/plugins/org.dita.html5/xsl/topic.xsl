...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="*[contains(@class, ' topic/image ')]/@scale">
      ...
      <xsl:if test="not(../@width) and not(../@height)">
        <xsl:attribute name="height" select="floor(number($height-in-pixel) * number($scale) div 100)"/>
        <xsl:attribute name="width" select="floor(number($width-in-pixel) * number($scale) div 100)"/>
      </xsl:if>
  </xsl:template>
  <!-- AM: handling for scale attribute on imagemap -->
  <xsl:template match="*[contains(@class, ' ut-d/imagemap ')]/@scale">
      <xsl:variable name="image" select="../*[contains(@class, ' topic/image ')]"/>
      <xsl:variable name="width-in-pixel">
        <xsl:call-template name="length-to-pixels">
          <xsl:with-param name="dimen" select="$image/@dita-ot:image-width"/>
        </xsl:call-template>
      </xsl:variable>
      <xsl:variable name="height-in-pixel">
        <xsl:call-template name="length-to-pixels">
          <xsl:with-param name="dimen" select="$image/@dita-ot:image-height"/>
        </xsl:call-template>
      </xsl:variable>
      <xsl:if test="not(@width) and not(@height)">
